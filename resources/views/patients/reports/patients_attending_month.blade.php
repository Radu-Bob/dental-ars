...
<div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
    ...
    <form method="GET" action="{{ route('reports.patients_attending.month_attendance') }}"
    ...
    <button type="button"
            onclick="(function(){
                ...
                if (k !== null) {
                    document.getElementById('month-export-key').value = k;
                    document.getElementById('month-export-form').submit();
                }
            })();"
        ...
        <p class="mt-0.5">
            <a href="#"
               onclick="(function(){
                   ...
                   if (k !== null && k.trim() !== '') {
                       document.getElementById('month-all-key').value = k;
                       document.getElementById('month-all-form').submit();
                   }
               })(); return false;"
        ...
        <table class="w-full text-sm border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                @foreach ($records as $record)
                    ...
                    {{-- Estimate row (only in all-records mode) --}}
...
@if (! $showAll)
<form id="month-all-form" method="POST"
      action="{{ route('reports.patients_attending.month_attendance.all') }}"
      style="display:none;">
    @csrf
    <input type="hidden" name="month" value="{{ $month }}">
    <input type="hidden" name="year"  value="{{ $year }}">
    <input type="hidden" name="access_key" id="month-all-key">
</form>
@endif

{{-- ── Hidden form: export ──────────────────────────────────────────────────── --}}
<form id="month-export-form" method="POST"
      action="{{ route('reports.patients_attending.month_attendance.export') }}"
      style="display:none;">
    @csrf
    <input type="hidden" name="month" value="{{ $month }}">
    <input type="hidden" name="year"  value="{{ $year }}">
    <input type="hidden" name="access_key" id="month-export-key">
</form>
