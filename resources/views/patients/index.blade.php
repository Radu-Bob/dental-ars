...
<div class="bg-white p-6 rounded-xl shadow-md space-y-4">
        ...
        <form action="{{ route('patients.index') }}" method="GET">
            <div class="mb-4">
                ...
                {{-- Updated Input: Removed 'focus:ring-blue-500' and added inline style for focus color --}}
            ...
            {{-- Updated Button: Uses the Lighter 'Standard' class --}}
            <button type="submit" 
                class="w-full btn-clinic-standard font-semibold py-2 rounded-lg shadow-md">
                Search
            </button>
        </form>


        {{-- 1. View All Records (Secondary Action) --}}
        <div class="mt-6">
        ...
        {{-- 2. Register New Patient (Primary Action) --}}
        <div class="mt-2"> 
        ...
        <div class="mt-4"> 
            <a href="{{ route('patients.partner.search') }}" 
            class="w-full text-center block font-semibold py-2 rounded-lg shadow-sm transition duration-300 bg-clinic-secondary hover:bg-clinic-secondary-hover">
    ...
    <div class="bg-white p-6 rounded-xl shadow-md">
        ...
        <ul class="list-unstyled space-y-2">
            @if($lastUpdatedPatients->count() > 0)
                @foreach($lastUpdatedPatients as $patient)
                @php
                    $timestamp = \Carbon\Carbon::parse($patient->time_stamp);
                @endphp

                @if($timestamp->format('Y-m-d') !== $lastDisplayedDate)
                    <li class="py-2 text-sm text-gray-500">
                        <span class="font-semibold text-clinic-bold">
                            @if($timestamp->isToday())
                                Today
                            @elseif($timestamp->isYesterday())
                                Yesterday
                            @else
                                {{ $timestamp->format('j M Y') }}
                            @endif
                        </span>
                    </li>
                    @php
                        $lastDisplayedDate = $timestamp->format('Y-m-d');
                    @endphp
                @endif
        ...
        <div class="overflow-x-auto bg-white rounded-lg shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        {{-- Sortable headers with dynamic links --}}
                        
                        <!--<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route('patients.index', ['q' => request('q'), 'sort_by' => 'patient_id', 'sort_order' => request('sort_order', 'asc') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center">
                                ID
...
-->
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route('patients.index', ['q' => request('q'), 'sort_by' => 'name', 'sort_order' => request('sort_order', 'asc') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center">
                                Name
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <a href="{{ route('patients.index', ['q' => request('q'), 'sort_by' => 'tel', 'sort_order' => request('sort_order', 'asc') == 'asc' ? 'desc' : 'asc']) }}" class="flex items-center">
                                Telephone
