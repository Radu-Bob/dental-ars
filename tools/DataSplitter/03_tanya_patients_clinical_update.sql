...
-- ─────────────────────────────────────────────────────────────────────────────
-- 03 — Tanya clinical sync: dtdc2 → 792088_tanya_decrypted
...
INSERT INTO `792088_tanya_decrypted`.`patients_clinical` (
...
WHERE (p.`active` = 1 OR p.`active` <> 2)
  AND NOT EXISTS (
      SELECT 1 FROM `792088_tanya_decrypted`.`patients_clinical` AS dest
